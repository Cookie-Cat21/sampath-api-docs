"""Minimal unofficial HTTP client for Sampath Bank API.
...
DEFAULT_UA = "sampath-api-docs-unofficial/0.1 (+https://github.com/Cookie-Cat21/sampath-api-docs; educational; polite)"
DEFAULT_BASE = "https://www.sampath.lk"


class SampathApiDocsClient:
    """Unofficial client — Sampath Bank API."""
    ...
    def __enter__(self) -> "SampathApiDocsClient":
        return self
    ...
    def rates_and_charges_external(self, **kwargs: Any) -> Any:
    ...
    def card_promotions_super_markets(self, page_number: int = 1, size: int = 20, **kwargs: Any) -> Any:
        """Supermarket card offers JSON.

        Catalog id: `card_promotions_super_markets` · GET
        """
        url = self._with_query("https://www.sampath.lk/api/card-promotions?category=super_markets&page_number=1&size=20", {'page_number': page_number, 'size': size})
        payload: Any = None
        return self._request_json(url, method='GET', **kwargs)
